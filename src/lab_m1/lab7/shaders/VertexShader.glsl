...
void main()
{
    ...
    diffuse_light = material_kd * intensitateLumina * max (dot(world_normal, L), 0);
    ...
    if (diffuse_light > 0)
    {
        specular_light = material_ks * intensitateLumina * pow(max(dot(V, R), 0), material_shininess);
    }

    // TODO(student): Compute light
    float dist = distance(light_position, world_position);
    float light = ambient_light + (1 / (dist * dist + 1)) * (diffuse_light + specular_light);
