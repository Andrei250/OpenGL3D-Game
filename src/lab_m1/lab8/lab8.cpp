...
{
    {
        ...
        RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], modelMatrix, glm::vec3(0.1f, 0.8f, 0.5f));
...

    {
        ...
        // TODO(student): Add or change the object colors
        RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix, glm::vec3(0.5f, 0.3f, 0.5f));
    ...
    // Render ground
    {
        ...
        // TODO(student): Add or change the object colors
        RenderSimpleMesh(meshes["plane"], shaders["LabShader"], modelMatrix, glm::vec3(0.1f, 0.2f, 0.5f));
...
void Lab8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    if (spot_light_mode) {
        int spotlight_mode = glGetUniformLocation(shader->program, "spotlight_mode");
        glUniform1i(spotlight_mode, spot_light_mode);

        float spotlightRadius = glGetUniformLocation(shader->program, "spotlight_radius");
        glUniform1f(spotlightRadius, spotlihght_radius);
    }
    else {
        int spotlight_mode = glGetUniformLocation(shader->program, "spotlight_mode");
        glUniform1i(spotlight_mode, 0);
    }
    // Bind model matrix
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->KeyHold(GLFW_KEY_UP)) spotlihght_radius += deltaTime * SPOTLIGHT_SPEED;
        if (window->KeyHold(GLFW_KEY_DOWN)) spotlihght_radius -= deltaTime * SPOTLIGHT_SPEED;
    }
}
...
void Lab8::OnKeyPress(int key, int mods)
{
    ...
    if (key == GLFW_KEY_F) {
        if (spot_light_mode == false) {
            spot_light_mode = true;
        }
        else {
            spot_light_mode = false;
        }
    }
}
